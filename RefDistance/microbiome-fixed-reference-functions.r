...
HMPdistance <- function(tax.level = NULL,                 # one of: 'l2.phylum', 'l3.class', 'l4.order', 'l5.family', 'l6.genus'
    ...
    colnames(hmp.dist) <- c('BC dist.to.ref.stool', 'BC dist.to.ref.nasal')
...
#Update HMP distance function based on Unifrac distance
hmp.distance.helper.new <- function(d.ref,d.new, measure = "uw")
{
  n <- length(d.new)
  n.ref <- length(d.ref)
  out <- matrix(NA,n,n.ref)
  
  if(tolower(measure) == "uw"){
    for(i in 1:n){
      for(j in 1:n.ref){
       
       if(d.ref[j] %in% d.new)
       {
         out[i,j] = unifrac_uw[d.new[i],d.ref[j]] 
       }
      }
        
    }
  }
  else if(tolower(measure) == "wu"){
    for(i in 1:n){
      for(j in 1:n.ref){
        
        if(d.ref[j] %in% d.new)
        {
          out[i,j] = unifrac_wu[d.new[i],d.ref[j]] 
        }
      }
    }
    }
  else{
      print('error in HMPdistance() - option for measure not recognized, use either "uw" for Unweighted Unifrac or "wu" for Weighted Unifrac')
  }
  
  
  out.summary <- rowMeans(out,na.rm = TRUE)
                                          ...
                                          print.details)            # = F) # order of columns corresponds to columns in gg.taxonomy.file
{
    ...
    if(substr(d.new.filename, nchar(d.new.filename)-2, nchar(d.new.filename)) == 'csv'){
        ...
        d.new <- read.table(d.new.filename,check.names=F,header = T)
    }
...
##################################################
# FUNCTIONS FOR COMPUTING Unifrac Distance #
# ################################################

#Input:
# biom_file: File name for biom file.
# tree_file: File name for phylogentic tree
# tax.level: Depending on biom file used, choose tax level for the analysis.
# distance_measure: "uw" for Unweighted Unifrac or "wu" for Weighted Unifrac
#output:
#return distance matrix.

Calculate_Unifrac <- function(biom_file, tax.level, tree_file, distance_measure)
{
  library(phyloseq)
  library(GUniFrac)
  ...
  source("GUniFrac.R")
  #tax.level <- "Rank2"
  #tree_file <- "rep_set_v35.tre"
  #distance_measure <- "uw" 
  #biom_file <- "HMP_YW_even1000.biom"
  
  tree <- read_tree_greengenes(tree_file)
  ...
  biomfile <- import_biom(biom_file)   #Rarefaction happens here.
  ...
  if(tolower(distance_measure) == "uw" )
  {
    unifrac <- unifracs$unifracs[,,4]  #d_UW: unweighted unifrac
  }
  else if(tolower(distance_measure) == "wu")
  {
    unifrac <- unifracs$unifracs[,,3]  #d_1: weighted unifrac
  }
  else
  {
    print('option for distance measure not recognized, use either "uw" for Unweighted Unifrac or "wu" for Weighted Unifrac')
  }
...
##################################################
# FUNCTIONS FOR COMPUTING HMP Distance #
# ################################################
# input:
# Level = taxonomy level. One of the following "l2.phylumn","l3.class","l4.order","l5.family","l6.genus"
# data: data frame with n (number of samples ) and p taxa. For each row, it should be added to 1
# distance_measure = three measure: one of the following "Bray_Curtis", "Unweighted_Unifrac" and "Weighted_Unifrac"
##
# output:
# Depending on distance measures it choose, it will report mean distance to HMP stool reference and HMP nasal reference. 
# To be completed.

HMPdistance_new <- function(tax.Level, data, distance_measure)
{
  tax.level <- c("l2.phylum","l3.class","l4.order","l5.family","l6.genus")
  
  #load(data)
  
  if(distance_measure == "Bray_Curtis")
  {
    #generate relative distance based on biom file
    #tax.level = "Rank2"
    #biomfile <- import_biom("HMP_YW_even1000.biom")
    
    #phyloseq_summarize_taxa(biomfile,tax.level)
    #biomfile <- tax_glom(biomfile,tax.level)  #Collapse into different taxonomy level if level2 using "Rank2"
    #otu.table <- otu_table(biomfile,taxa_are_rows = TRUE)
    
    #taxtable <- tax_table(biomfile)
    #taxtable <- as.data.frame(taxtable@.Data)
    
    
    #otu.table <- otu.table@.Data
    #otu.table <- t(otu.table)
    
    hmp.dist <-  HMPdistance(tax.level, d.new.filename = data, d.new.ix.col.not.rel.abu = 1:7, measure = 'bc')
    colnames(hmp.dist) <- c('dist.to.hmp.stool', 'dist.to.hmp.nasal')
    
  }
  elsif (distance_measure == "Unweighted_Unifrac")
  {
    #Step 1: Find Unifrac distance corresponding in corresponding level
    
    
    # tax.level = "l2.phylum"
    
    #  load(paste0("Unweighted_Unifrac_",tax.level,".RData"))
    
    unifrac_uw = Calculate_Unifrac("HMP_YW_even1000.biom","Rank2","rep_set_v35.tre","uw")
    
    
    #Step 2: Pick Run_test function to select HMP stool and HMP nasal distance.
    
    
    sample_list <- rownames(unifrac_uw)
    
    
    #run_test_new(tax.level = "l2.phylum", sample_list, measure = "uw")
  ...
  elsif (distance_measure == "Weighted_Unifrac")
  {
    
    #tax.level = "l2.phylum"
    #load(paste0("Weighted_Unifrac_",tax.level,".RData"))
