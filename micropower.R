...
readDM <- function(filepath=file.choose()) {
  dm <- as.matrix(read.delim(filepath,header=T,row.names=1,check.names=FALSE))
  return(dm)
}
...
calcOmega2 <- function(dm) {
  ...
  ssw <- sum(unlist(sapply(dm_within,FUN=function(x) {sum(lowerTriDM(x)^2)/nrow(x)})))
  ssa <- sst-ssw
  group_ms <- ssa/df
